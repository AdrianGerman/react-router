...
function DataProvider({ children }) {
  ...
  const deleteData = (text) => {
    const obj1 = data.find((e) => e.title === text);
    console.log(obj1);
    setBlogdata([...obj1]);
    navigate("/blog");
  };
  ...
  const editData = (array) => {
    ...
    const editedArray = data.map((e) =>
      e.title === array[0] ? editedElement : e
    );
    setBlogdata([...editedArray]);
    navigate("/blog");
  };
  ...
  return (
    <dataContext.Provider value={blogdata}>{children}</dataContext.Provider>
  );
}
